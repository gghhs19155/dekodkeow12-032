// === КОНФИГУРАЦИЯ ===
const SPREADSHEET_ID = '1QX07u6LhEViR6xrRh6xzywvgB2k5kYmjU3uRWvMhymI'; // ← ТВОЙ ID!
const TELEGRAM_TOKEN = '8458389593:AAH_7DIWUV4EvwYNuMAW4fO2eBjk9Xhv0HM'; // ← ТВОЙ ТОКЕН!
const TELEGRAM_CHAT_ID = '7198093055'; // ← ТВОЙ CHAT ID!
...
async function loadClients() {
  try {
    const response = await fetch(
      `https://sheets.googleapis.com/v4/spreadsheets/${SPREADSHEET_ID}/values/Clients?alt=json`
    );
...
async function loadAccounts() {
  try {
    const response = await fetch(
      `https://sheets.googleapis.com/v4/spreadsheets/${SPREADSHEET_ID}/values/Accounts?alt=json`
    );
    const data = await response.json();
...
async function addClient() {
  ...
  try {
    const response = await fetch(
      `https://sheets.googleapis.com/v4/spreadsheets/${SPREADSHEET_ID}/values/Clients:append?valueInputOption=RAW`,
      {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
...
async function checkAllClients() {
  ...
  for (let i = 0; i < clients.length; i++) {
    ...
    if (client.status === 'Не проверено') {
      ...
      try {
        await fetch(
          `https://sheets.googleapis.com/v4/spreadsheets/${SPREADSHEET_ID}/values/Clients!D:D?range=D${i+2}&valueInputOption=RAW`,
          {
            method: 'PUT',
            headers: { 'Content-Type': 'application/json' },
...
async function updateAccountStatus() {
  ...
  try {
    await fetch(
      `https://sheets.googleapis.com/v4/spreadsheets/${SPREADSHEET_ID}/values/Accounts!B:C?valueInputOption=RAW`,
      {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
